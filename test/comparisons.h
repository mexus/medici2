...
bool Compare(const std::set<T>& etalon, const std::set<T>& test) {
    ...
    for (auto el : etalon) {
        ...
        if (itTest == test.end()) {
            log(logxx::error) << "Element {" << el << "} not found in a test set"
                              << logxx::endl;
            res = false;
        }
    }
    for (auto el : test) {
        ...
        if (itEtalon == etalon.end()) {
            log(logxx::error) << "Extra element {" << el << "} in a test set"
                              << logxx::endl;
            res = false;
        }
    }
...
bool Compare(const std::map<T1, T2>& etalon, const std::map<T1, T2>& test) {
    ...
    for (auto pair : etalon) {
        ...
        if (itTest == test.end()) {
            ...
            log(logxx::error) << "Element {" << el << "} has value {" << pair.second
                              << "} in etalon, but {" << itTest->second
                              << "} in a test set" << logxx::endl;
            res = false;
        }
    }
    ...
    for (auto pair : test) {
        ...
        if (itEtalon == etalon.end()) {
            ...
            log(logxx::error) << "Element {" << el << "} has value {" << itEtalon->second
...
bool Compare(const std::array<T, N>& etalon, const std::array<T, N>& test) {
    ...
    for (std::size_t i = 0; i != N; ++i) {
        ...
        auto& testEl = test[i];
        if (!(etalonEl == testEl)) {
            log(logxx::error) << "Error at element #" << i << ": got {" << testEl
                              << "}, but should be {" << etalonEl << "}" << logxx::endl;
            res = false;
        }
    }
    return res;
